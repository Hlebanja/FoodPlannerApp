<template>
  <div class="jumbotron feedback">
    ...
    <b-list-group>
      <feedback-item v-for="feedback in feedbacks" :key="feedback._id" :feedback="feedback" @delete-feedback="deleteFeedback"></feedback-item>
    </b-list-group>
  </div>
</template>
...
<script>
import { Api } from '@/Api'
import FeedbackItem from '@/components/AdminFeedbackItem'
...
export default {
  ...
  methods: {
    deleteFeedback(id) {
      Api.delete(`/feedbacks/${id}`).then(response => {
        console.log(response.data.message)
        var index = this.feedbacks.findIndex(camel => camel._id === id)
